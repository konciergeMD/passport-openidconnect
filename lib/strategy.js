          ...
          if (load) {
            ...
            oauth2._request(
				...
				function (err, body, res) {
	              if (err) {
					  console.log('Request to ', userInfoURL, ', failed with status ', res.statusCode);
					  console.log(err); // acp koa logger removes stack trace
					  ...
					  userInfo = oauth2._request(
						...
						function (err, redirectBody, redirectRes) {
							if (err || !(redirectRes?.statusCode >= 200 && redirectRes?.statusCode <= 299)) {
								console.log('Redirect request to ', redirectRes?.headers?.location, ', failed with status ', redirectRes?.statusCode);
								console.log(err); // acp koa logger removes stack trace
