 ...
 * @api public
 */
function AcpStrategy(options, verify) {
  options = options || {}
 ...
 */
util.inherits(AcpStrategy, passport.Strategy);
 ...
 * @api protected
 */
AcpStrategy.prototype.authenticate = function(req, options = {}) {
  var self = this;
  ...
  if (req.query && req.query.code) {
		...
		*/
        ...
        self._shouldLoadUserProfile(iss, sub, function(err, load) {
          ...
          if (load) {
			...
			// console.log('Making request too: ', userInfoURL, ' with Bearer ', accessToken);
            oauth2._request(
				"GET",
				userInfoURL,
				{
				...
				function (err, body, res) {
	              if (err) {
					  console.error('Request to ', userInfoURL, ', failed with status ', res && res.statusCode);
					  console.error(err); // acp koa logger removes stack trace
					  return self.error(new InternalOAuthError('failed to fetch user profile location', err));
				  }
				  // Accolade OIC will redirect to a new location
				  // make another call to the response.header.location for user info...
				  var userInfo = {};
          if (res.statusCode === 200) {
            ...
            onProfileLoaded(profile);
					  ...
					  userInfo = oauth2._request(
						...
						null,
						null,
						function (err, redirectBody, redirectRes) {
							if (err || !(redirectRes.statusCode >= 200 && redirectRes.statusCode <= 299)) {
								console.error('Redirect request to ', res && res.headers && res.headers.location, ', failed with status ', redirectRes && redirectRes.statusCode);
								console.error(err); // acp koa logger removes stack trace
						   ...
						   try {
							 var json = JSON.parse(redirectBody);
					  ...
					  return self.error(new InternalOAuthError('failed to fetch user profile after redirect'));
				  }
            });
          } else {
            onProfileLoaded();
    ...
    this.configure(identifier, function(err, config) {
      ...
      if (options.extraParams) {
        Object.keys(options.extraParams).forEach(paramName => params[paramName] = options.extraParams[paramName]);
      }
 ...
 * @api public
 */
AcpStrategy.prototype.configure = function(identifier, done) {
 ...
 * @api protected
 */
AcpStrategy.prototype.authorizationParams = function(options) {
	let params = {};
	const prompt = options.prompt;
	params = prompt ? Object.assign(params, { prompt: prompt }) : params;
 ...
 * @api private
 */
AcpStrategy.prototype._shouldLoadUserProfile = function(issuer, subject, done) {
