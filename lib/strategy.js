          ...
          if (load) {
            ...
            oauth2._request(
				...
				function (err, body, res) {
	              if (err) {
					  console.error('Request to ', userInfoURL, ', failed with status ', res?.statusCode);
					  ...
					  userInfo = oauth2._request(
						...
						function (err, redirectBody, redirectRes) {
							if (err || !(redirectRes?.statusCode >= 200 && redirectRes?.statusCode <= 299)) {
								console.error('Redirect request to ', redirectRes?.headers?.location, ', failed with status ', redirectRes?.statusCode);
								console.error(err); // acp koa logger removes stack trace
